...
conversion.add("1", "2", function(data, callback) {
  ...
  validate(data.data, function(err) {
    if (err) {
      callback(err);
    } else {
 ...
 * @param {ConversionCallback} callback - The callback that receives
 *   the result of conversion and validation.
 */
  ...
  if (fn) {
    ...
    fn(data, function(err) {
      if (err) {
        ...
        validate(data.data, function(err, version) {
          if (err) {
            callback(err);
          } else if (version !== CURRENT) {
            callback(new Error("Replay not fully transformed."));
          } else {
            callback(null);
          }
        });

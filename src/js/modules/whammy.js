...
module.exports = (function(){
    ...
    for(let i = 0;i < frames;i++) {
      if(frames[i].frameNum!=i){
        console.log('Frame'+frames[i].frameNum+' is at '+i);
      }
    }
  ...
  function parseWebP(riff){
    /*if(!window.getoutter) {
      window.outerriff = riff;
      console.log(window.outerriff);
      window.getoutter = true;
    }*/
    return riff.RIFF[0].then(function(RIFF) {
      /*if(!window.getout1) {
        window.RIFF = RIFF;
        console.log(window.RIFF);
        console.log(window.RIFF.WEBP);
        window.getout1 = true;
      }
      if(!RIFF.WEBP || !RIFF.WEBP[0]) {
        if(!window.getout2) {
          window.getout2 = true;
          console.log('had to escape',window.getout1,window.outerriff);
        }
        return {width,height,riff};
      }*/

...
function recordReplayData() {
  ...
  var saveFrame = function () {
    ...
    data.dynamicTiles.forEach((tile) => {
      tile.value.shift();
...
var TagProReplays = {
  ...
  capture: function (opts) {
    ...
    logger.info('Sending replay data from injected script to content script.');
    var replay = {
      data: data,
      info: info,
      version: "2"
...
DOMMessaging.listen('replaySaved', (err) => {
  logger.info('Got confirmation message.');
  if (err) {
    $("#savedFeedback").text("Failed!");
    $("#savedFeedback").css({
      color: "red"
    });
    logger.warn("Saving failed because: " + err.reason);
  } else {
...
// function to add button to record replay data AND if user has turned on key
// recording, add listener for that key.
